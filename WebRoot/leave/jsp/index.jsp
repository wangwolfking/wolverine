...
<body>
	<a href="deployFlow.action">发布流程请假流程</a> username:
	<c:out value="${username }"></c:out>
	<a href="login/jsp/login.jsp">登录</a>
	<br>
	<br>
	<br>
	<fieldset>
		...
		<table>
			<c:if test="${processDefinitionList!=null && fun:length(processDefinitionList) > 0}">
				<tr>
					<th>流程id</th>
					<th>流程名称</th>
					<th>发布id</th>
				...
				<c:forEach items="${processDefinitionList}" var="pd">
					<tr>
						...
						<td><a href="deleteFlow.action?deploymentId=${pd.deploymentId }">删除</a> <a href="startFlow.action?key=${pd.key }">我要请假</a></td>
					</tr>
				</c:forEach>
			</c:if>
			<c:if test="${processDefinitionList==null || fun:length(processDefinitionList) <= 0}">
	...
	<fieldset>
		...
		<table>
			<c:if test="${processInstanceList!=null && fun:length(processInstanceList) > 0}">
				<tr>
					<th>实例id</th>
					<th>实例名称</th>
					<th>businessKey</th>
	...
	<fieldset>
		...
		<table>
			<c:if test="${historicTaskList!=null && fun:length(historicTaskList) > 0}">
				<tr>
					<th>任务id</th>
					<th>任务名称</th>
					<th>创建时间</th>
				...
				<c:forEach items="${historicTaskList}" var="tl">
					<tr>
						...
						<td><a target="_blank" href="${pageContext.request.contextPath}/queryProPlan.action?processDefinitionid=${tl.processDefinitionId }&ProcessInstanceId=${tl.processInstanceId	}">view</a></td>
					</tr>
				</c:forEach>
			</c:if>
			<c:if test="${historicTaskList==null || fun:length(historicTaskList) <= 0}">
	...
	<fieldset>
		...
		<table>
			<c:if test="${unhistoricTaskList!=null && fun:length(unhistoricTaskList) > 0}">
				<tr>
					<th>任务id</th>
					<th>任务名称</th>
					<th>创建时间</th>
				...
				<c:forEach items="${unhistoricTaskList}" var="tl">
					<tr>
						...
						<td><c:out value="${tl.processDefinitionId }"></c:out></td>
						<td><a href="${pageContext.request.contextPath}/${tl.formKey }?taskid=${tl.id }&processInstanceId=${tl.processInstanceId }">请假</a></td>
						<td><a target="_blank" href="${pageContext.request.contextPath}/queryProPlan.action?processDefinitionid=${tl.processInstanceId }">view</a></td>
					</tr>
				</c:forEach>
			</c:if>
			<c:if test="${unhistoricTaskList==null || fun:length(unhistoricTaskList) <= 0}">
	...
	<fieldset>
		...
		<table>
			<c:if test="${tasklist!=null && fun:length(tasklist) > 0}">
				<tr>
					<th>任务id</th>
					<th>任务名称</th>
					<th>创建时间</th>
